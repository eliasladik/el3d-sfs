...
async function openQrLabel(id, pieceNumber) {
    ...
    try {
        ...
        document.getElementById('qr-label-meta').textContent = `${spool.material_type} · ${spool.color_name} · Kus ${pieceNumber}`;
        document.getElementById('qr-image').src = qr.dataUrl;
        document.getElementById('qr-payload').textContent = `Interní kód: ${qr.payload}`;
...
function handleQrPayload(payload) {
    const match = /^EL3D-SFS:(\d+)$/.exec(String(payload).trim());
    if (!match) { document.getElementById('scanner-status').textContent = 'Toto není QR kód EL3D SFS.'; return false; }
    const id = Number(match[1]); const spool = mockSpools.find(item => item.id === id);
    if (!spool) { document.getElementById('scanner-status').textContent = 'Cívka není v tvém regálu.'; return false; }
    closeScanner(); currentFilter = 'ALL'; searchQuery = ''; document.getElementById('search-input').value = '';
    const key = `${spool.brand}-${spool.material_type}-${spool.color_name}`.toLowerCase(); expandedCards[key] = true; renderSpools();
    const card = document.querySelector(`[data-spool-ids~="${id}"]`); if (card) card.scrollIntoView({ behavior: 'smooth', block: 'center' });
    return true;
}
