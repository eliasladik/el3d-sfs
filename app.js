...
async function handleSignup(e) {
    ...
    if (pass.length < 3) { errorBox.innerText = "Heslo musí mít aspoň 3 znaky!"; errorBox.classList.remove('hidden'); return; }
...
async function changeSystemPassword() {
    ...
    if (newPass.length < 3) { alert("Slabé heslo! Musí mít minimálně 3 znaky."); return; }
    if (newPass !== newPassConfirm) { alert("Zadaná nová hesla se neshodují! Zkontroluj překlepy."); return; }

    systemPassword = newPass; await persistConfig();
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (sessionActive === 'true' && currentUser) {
        ...
        fetch(`${API_URL}/auth`, {
            ...
            materialTypes = data.config.materialTypes; lowStockLimit = data.config.lowStockLimit; systemPassword = data.config.systemPassword; defaultExpandMode = data.config.defaultExpandMode;
            loadDataFromServer();
        });
...
async function persistConfig() {
    try {
        const payload = { user: currentUser, config: { materialTypes, lowStockLimit, systemPassword, defaultExpandMode } };
...
async function handleLogin(e) {
    ...
    if(!user) return;
    try {
        const authRes = await fetch(`${API_URL}/auth`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: user, password: pass }) });
        const data = await authRes.json();
        if (data.success) {
            errorBox.classList.add('hidden'); currentUser = user;
            localStorage.setItem('sfs_current_user', currentUser);
            if (document.getElementById('login-remember').checked) localStorage.setItem('sfs_session_active', 'true');
            materialTypes = data.config.materialTypes; lowStockLimit = data.config.lowStockLimit; systemPassword = data.config.systemPassword; defaultExpandMode = data.config.defaultExpandMode;
            stopParticleAnimation(); document.getElementById('login-page').classList.add('opacity-0');
            setTimeout(() => { document.getElementById('login-page').classList.add('hidden'); document.getElementById('app-content').classList.remove('hidden', 'opacity-0'); loadDataFromServer(); }, 300);
        } else { errorBox.innerText = data.message || "Nesprávné heslo!"; errorBox.classList.remove('hidden'); }
    } catch (err) { errorBox.innerText = "Nelze se spojit se serverem."; errorBox.classList.remove('hidden'); }
}

function handleLogout() {
    localStorage.removeItem('sfs_session_active'); localStorage.removeItem('sfs_current_user'); currentUser = ''; 
...
function startParticleAnimation() { const canvas = document.getElementById('login-particles'); if(!canvas) return; const ctx = canvas.getContext('2d'); function resize() { canvas.width = window.innerWidth; canvas.height = window.innerHeight; } window.addEventListener('resize', resize); resize(); const particles = []; const particleCount = Math.min(80, Math.floor((canvas.width * canvas.height) / 22000)); for (let i = 0; i < particleCount; i++) { particles.push({ x: Math.random() * canvas.width, y: Math.random() * canvas.height, vx: (Math.random() - 0.5) * 0.4, vy: (Math.random() - 0.5) * 0.4, radius: Math.random() * 1.5 + 1 }); } function animate() { ctx.clearRect(0, 0, canvas.width, canvas.height); particles.forEach((p, idx) => { p.x += p.vx; p.y += p.vy; if (p.x < 0 || p.x > canvas.width) p.vx *= -1; if (p.y < 0 || p.y > canvas.height) p.vy *= -1; ctx.beginPath(); ctx.arc(p.x, p.y, p.radius, 0, Math.PI * 2); ctx.fillStyle = 'rgba(148, 163, 184, 0.4)'; ctx.fill(); for (let j = idx + 1; j < particles.length; j++) { const p2 = particles[j]; const dist = Math.hypot(p.x - p2.x, p.y - p2.y); if (dist < 130) { ctx.beginPath(); ctx.moveTo(p.x, p.y); ctx.lineTo(p2.x, p2.y); ctx.strokeStyle = `rgba(220, 38, 38, ${0.12 * (1 - dist / 130)})`; ctx.lineWidth = 0.6; ctx.stroke(); } } }); particleAnimationId = requestAnimationFrame(animate); } animate(); }
...
function renderSpools() {
    ...
    keys.sort().forEach(key => {
        ...
        const card = document.createElement('div'); card.className = `bg-slate-900 border ${isFullyEmpty ? 'border-slate-850 opacity-40 grayscale' : hasLowSpool ? 'border-red-900/40 ring-1 ring-red-950/20' : isExpanded ? 'border-slate-700 ring-1 ring-slate-800' : 'border-slate-800'} rounded-xl p-5 flex flex-col justify-between shadow-sm cursor-pointer select-none transition-all duration-200 hover:scale-[1.01]`; card.setAttribute('onclick', `toggleCard('${key}')`);
        let stackHtml = '';
        group.items.forEach((spool, idx) => {
            ...
            stackHtml += `<div class="bg-slate-950 border ${isItemEmpty ? 'border-slate-900' : isItemLow ? 'border-red-950 bg-red-950/5' : 'border-slate-850'} p-2.5 rounded-lg flex items-center justify-between gap-2 text-xs animate-fadeIn"><div class="flex flex-col gap-1 flex-1"><div class="flex justify-between font-mono text-slate-400"><span class="${isItemEmpty ? 'line-through text-slate-600' : 'text-slate-400'}">Kus: #${idx + 1} ${warningIndicator}</span><span class="${isItemEmpty ? 'text-slate-600' : isItemLow ? 'text-amber-500 font-bold' : 'text-slate-200'} font-sans font-medium">${isItemEmpty ? '0g' : spool.remaining_weight_g + 'g / ' + spool.total_capacity + 'g'}</span></div><div class="w-full bg-slate-900 h-1 rounded-full overflow-hidden"><div class="h-full ${isItemEmpty ? 'bg-slate-800' : isItemLow ? 'bg-amber-500' : 'bg-slate-500'}" style="width: ${itemPct}%"></div></div></div><div class="flex gap-1 pl-2" onclick="event.stopPropagation();"><button onclick="manualDeduct(${spool.id})" ${isItemEmpty ? 'disabled class="p-1.5 text-slate-800 cursor-not-allowed"' : 'class="p-1.5 bg-slate-900 hover:bg-slate-850 border border-slate-800 text-slate-300 rounded cursor-pointer transition-colors"'}><i data-lucide="minus-square" class="w-3.5 h-3.5"></i></button><button onclick="deleteSpool(${spool.id})" class="p-1.5 bg-slate-900 hover:bg-red-950/40 border border-slate-800 text-slate-500 hover:text-red-400 rounded cursor-pointer transition-colors"><i data-lucide="trash-2" class="w-3.5 h-3.5"></i></button></div></div>`;
        });
...
async function handleFormSubmit(e) {
    ...
    await fetch(`${API_URL}/spools`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ spools: tempBatch, user: currentUser }) });
    closeModal(); const spoolsRes = await fetch(`${API_URL}/spools?user=${currentUser}`); mockSpools = await spoolsRes.json(); updateFormDatalists(); renderSpools();
}
async function manualDeduct(id) {
    ...
    if (amount && !isNaN(amount)) {
        ...
        if (spool) { 
            ...
            await fetch(`${API_URL}/spools/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ remaining_weight_g: spool.remaining_weight_g, user: currentUser }) });
            renderSpools(); if (spool.remaining_weight_g < lowStockLimit && oldWeight >= lowStockLimit) { triggerWebNotification(spool); }
        }
    }
}
async function deleteSpool(id) { if(confirm("Smazat cívku?")) { await fetch(`${API_URL}/spools/${id}?user=${currentUser}`, { method: 'DELETE' }); mockSpools = mockSpools.filter(s => s.id !== id); updateFormDatalists(); renderSpools(); } }

// PŘESUNUTO Z HTML: Globální funkce pro přepínání očí u hesel (plně oživená)
function togglePasswordVisibility(inputId, iconId) {
    ...
    if (input && icon) {
        ...
        lucide.createIcons(); // Znovu vybuduje Lucide uzly pro změnu ikonky
    }
}