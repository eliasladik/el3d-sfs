...
async function persistConfig() {
    try {
        const payload = { config: { materialTypes, lowStockLimit, defaultExpandMode, printers } };
...
function renderSpools() {
    ...
    const keys = Object.keys(stackedGroups).filter(key => {
        if (!lowStockOnly) return true;
        return stackedGroups[key].items.some(item => item.remaining_weight_g === 0 || (item.remaining_weight_g > 0 && item.remaining_weight_g < lowStockLimit));
    });
    if (keys.length === 0) {
        grid.innerHTML = lowStockOnly
            ? `<div class="col-span-full text-center py-12 text-slate-500 bg-slate-900/20 border border-dashed border-slate-800 rounded-xl">Žádné cívky s nízkým stavem — vše je v pořádku 🎉</div>`
            : `<div class="col-span-full text-center py-12 text-slate-500 bg-slate-900/20 border border-dashed border-slate-800 rounded-xl">Tvůj regál je prázdný. Přidej první cívky.</div>`;
    ...
    keys.sort().forEach(key => {
        ...
        group.items.forEach((spool, idx) => {
            ...
            const isReserved = Boolean(spool.reserved_by); const reservedByMe = spool.reserved_by === currentUser; const canRelease = reservedByMe || currentRole === 'admin';
            ...
            const reservedBadge = isReserved ? `<div class="flex items-center gap-1 mt-1 text-[10px] text-amber-400 bg-amber-950/30 border border-amber-900/40 px-1.5 py-0.5 rounded w-fit"><i data-lucide="lock" class="w-3 h-3"></i><span>Rezervoval(a) ${spool.reserved_by}${spool.reserved_note ? ' · ' + spool.reserved_note : ''}</span></div>` : '';
            const reserveButton = isReserved
                ? `<button onclick="${canRelease ? `releaseSpool(${spool.id})` : ''}" title="${canRelease ? 'Uvolnit rezervaci' : `Rezervoval(a) ${spool.reserved_by} - uvolnit může jen on(a) nebo admin`}" ${canRelease ? '' : 'disabled'} class="p-1.5 ${canRelease ? 'bg-amber-950/40 hover:bg-amber-900/40 text-amber-400 cursor-pointer' : 'bg-slate-900 text-slate-700 cursor-not-allowed'} border border-amber-900/40 rounded transition-colors"><i data-lucide="lock" class="w-3.5 h-3.5"></i></button>`
                : `<button onclick="openReserveModal(${spool.id})" title="Rezervovat pro tisk" class="p-1.5 bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-400 hover:text-white rounded cursor-pointer transition-colors"><i data-lucide="lock-open" class="w-3.5 h-3.5"></i></button>`;
            stackHtml += `<div id="spool-row-${spool.id}" class="bg-slate-950 border ${isReserved ? 'border-amber-900/60 ring-1 ring-amber-900/30' : isItemEmpty ? 'border-slate-900' : isItemLow ? 'border-red-950 bg-red-950/5' : 'border-slate-850'} p-2.5 rounded-lg flex items-center justify-between gap-2 text-xs animate-fadeIn transition-all duration-300"><div class="flex flex-col gap-1 flex-1"><div class="flex justify-between font-mono text-slate-400"><span class="${isItemEmpty ? 'line-through text-slate-600' : 'text-slate-400'}">Kus ${idx + 1} ${warningIndicator}</span><span class="${isItemEmpty ? 'text-slate-600' : isItemLow ? 'text-amber-500 font-bold' : 'text-slate-200'} font-sans font-medium">${isItemEmpty ? '0g' : spool.remaining_weight_g + 'g / ' + spool.total_capacity + 'g'}</span></div><div class="w-full bg-slate-900 h-1 rounded-full overflow-hidden"><div class="h-full ${isItemEmpty ? 'bg-slate-800' : isItemLow ? 'bg-amber-500' : 'bg-slate-500'}" style="width: ${itemPct}%"></div></div>${reservedBadge}</div><div class="flex gap-1 pl-2" onclick="event.stopPropagation();">${reserveButton}<button onclick="openQrLabel(${spool.id}, ${idx + 1})" title="Vytisknout QR štítek" class="p-1.5 bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-400 hover:text-white rounded cursor-pointer transition-colors"><i data-lucide="qr-code" class="w-3.5 h-3.5"></i></button><button onclick="manualDeduct(${spool.id})" ${isItemEmpty ? 'disabled class="p-1.5 text-slate-800 cursor-not-allowed"' : 'class="p-1.5 bg-slate-900 hover:bg-slate-850 border border-slate-800 text-slate-300 rounded cursor-pointer transition-colors"'}><i data-lucide="minus-square" class="w-3.5 h-3.5"></i></button><button onclick="deleteSpool(${spool.id})" class="admin-only p-1.5 bg-slate-900 hover:bg-red-950/40 border border-slate-800 text-slate-500 hover:text-red-400 rounded cursor-pointer transition-colors"><i data-lucide="trash-2" class="w-3.5 h-3.5"></i></button></div></div>`;
        });
