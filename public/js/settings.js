...
function renderPrintersManagerList() { const listContainer = document.getElementById('printers-list-manager'); if (!listContainer) return; listContainer.innerHTML = printers.length ? '' : '<span class="text-xs text-slate-500">Zatím žádné tiskárny. Přidej alespoň jednu, ať se dá vybrat při rezervaci cívky.</span>'; printers.forEach(printer => { listContainer.innerHTML += `<span class="inline-flex items-center gap-1.5 bg-slate-950 border border-slate-800 text-xs font-semibold px-2.5 py-1 rounded-md text-slate-300">${printer}<button onclick="removePrinter('${printer.replace(/'/g, "\\'")}')" class="text-slate-500 hover:text-red-400 transition-colors cursor-pointer"><i data-lucide="x" class="w-3 h-3"></i></button></span>`; }); lucide.createIcons(); }
async function addNewPrinter() { const input = document.getElementById('new-printer-input'); const value = input.value.trim(); if (value && !printers.includes(value)) { printers.push(value); input.value = ''; await persistConfig(); renderPrintersManagerList(); } }
async function removePrinter(name) { if(confirm(`Opravdu smazat tiskárnu "${name}"?`)) { printers = printers.filter(p => p !== name); await persistConfig(); renderPrintersManagerList(); } }
